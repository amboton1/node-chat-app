...
io.on('connection', (socket) => {
    ...
    socket.on('sendMessage', (message, callback) => {
        ...
        io.to(user.room).emit('message', generateNewMessage(message))
        callback()
    })
    ...
    socket.on('sendLocation', (location, callback) => {
        const user = getUser(socket.id)
        io.to(user.room).emit('location', generateLocationMessages(user.username, `https://google.com/maps?q=${location.latitude},${location.longitude}`))
        callback()
    })
})
